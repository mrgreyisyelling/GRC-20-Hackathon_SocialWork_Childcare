...
export class TransactionService {
     ...
     */
    static async submitOperations(spaceId, ops) {
        if (!spaceId) {
        ...
        try {
            ...
            try {
                ...
                cid = await Ipfs.publishEdit({
                    ...
                    ops: ops, // Type conversion needed due to SDK differences
     ...
     */
    static async submitOperationBatches(spaceId, opsBatches, batchSize = 100) {
        if (!spaceId) {
            throw new Error('Space ID is required');
        }
     ...
     */
    static splitIntoBatches(ops, batchSize = 100) {
        if (!ops || ops.length === 0) {
            return [];
        }
        const batches = [];
     ...
     */
    static async spaceExists(spaceId) {
        try {
